"""Clubes de barrio de CABA desde el registro abierto GCBA."""
...
def parse_args():
  p = argparse.ArgumentParser(description="Scraper GCBA Clubes de barrio (CABA)")
...
async def main():
 ...
 for f in fs:
  ...
  candidates.append({'name': name, 'entity_type': 'Facility', 'subtype': 'sports_club', 'lat': lat, 'lng': lng, 'origin_url': URL,
   'values': (('address', clean(x.get('direccion'))), ('phone', clean(x.get('telefono'))), ('website', clean(x.get('web'))), ('sports', clean(x.get('actividade'))), ('facilities', clean(x.get('instalacio'))), ('neighborhood', neighborhood), ('commune', commune))})
 candidates = bounded(candidates, args.limit)
 print(f"Clubes de barrio ({args.scope}): {len(candidates)}")
