"""Instituciones de colectividades de Palermo, fuente oficial GCBA."""
...
async def main():
 ...
 async with httpx.AsyncClient(timeout=60) as c:
  ...
  candidates=[]
  for x in rows:
   if clean(x.get('barrio')).upper()!='PALERMO' and clean(x.get('comuna')).upper()!='COMUNA 14': continue
   name=normalize_name(clean(x.get('nombre'))); address=f"{clean(x.get('calle_nombre'))} {clean(x.get('calle_altura'))}".strip()
   if not name: continue
   candidates.append({'name':name,'entity_type':'Organization','subtype':'community_institution','lat':num(x.get('lat')),'lng':num(x.get('long')),'origin_url':URL,
    'values':(('address',address),('phone',clean(x.get('telefono'))),('website',clean(x.get('web'))),('institution_type',clean(x.get('tipo'))),('community',clean(x.get('colectividad'))),('neighborhood','Palermo'))})
 print(f"Instituciones comunitarias de Palermo: {len(candidates)}")
