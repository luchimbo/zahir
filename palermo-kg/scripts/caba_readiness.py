...
async def main():
    ...
    async with pool.acquire() as conn:
        ...
        source_rows = await conn.fetch("""SELECT s.source_name, COALESCE(m.coverage, JSON_OBJECT()) coverage
            FROM sources s
            LEFT JOIN (
                SELECT r.source_id, MAX(r.id) AS id
                FROM source_sync_runs r
                JOIN (
                    SELECT source_id, MAX(started_at) AS latest_started_at
                    FROM source_sync_runs GROUP BY source_id
                ) latest ON latest.source_id=r.source_id AND latest.latest_started_at=r.started_at
                GROUP BY r.source_id
            ) latest_run ON latest_run.source_id=s.id
            LEFT JOIN source_run_metrics m ON m.run_id=latest_run.id
